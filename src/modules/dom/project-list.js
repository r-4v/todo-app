...
function addProjectListItem(projectName) {
  ...
  if (projectList) {
    projectList.forEach((project) => {
      ...
      projectListItemWrapper.setAttribute(
        "style",
        `display:flex;align-items:center;flex-wrap:wrap;`
      );
      projectDeleteButton.setAttribute(
        `style`,
        `height:20px;padding:5px;background-color:red;color:white;font-size:10px;font-weight:bold;border-radius:5px;border:none;margin-left:auto;margin-right:30px;`
      );
      projectDeleteButton.innerText = "Delete";
      projectDeleteButton.addEventListener("click", deleteProject);
      projectListItem.setAttribute(
        "style",
        `display:flex;color:white;font-weight:bold;font-size:18px;cursor:pointer;margin:10px;justify-content:center;align-items:center;background:black;padding:10px;border-radius:10px; `
      );
...
function deleteProject(e) {
  ...
  if(projectList){
  setProjectList(
    projectList.filter((project) => project.projectName !== projectToDelete)
  );}
...
function refreshProjectTaskList(e) {
  ...
  if (todoList) {
    filteredTodoList = todoList.filter(
      (todo) => todo.projectName === e.target.innerText
    );
    ...
    if (filteredTodoList) {
      filteredTodoList.map((todo) => {
        ...
        taskListItem.setAttribute(
          "style",
          `color:white;font-weight:bold;font-size:18px;margin:20px;cursor:pointer;`
        );
